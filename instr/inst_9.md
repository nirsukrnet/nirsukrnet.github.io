...
Develop a standalone HTML file (`html/vis_frame_controller.html`) to visualize the `FrameController` state mapping.

*   **Concept**: Use a static image (or placeholder div) to represent the audio track.
*   **Value Generation**:
    *   Simulate `track_Duration` based on the image width (e.g., 1 pixel = 0.1 seconds).
*   **Visual Elements**:
    *   **Global View**: A container representing the full track.
    *   **View Window**: An overlay div representing `start_pos_audioframe` / `end_pos_audioframe`.
    *   **Selection**: An overlay div representing `start_pos_audio_selection` / `end_pos_audio_selection`.
*   **Functionality**:
    *   Initialize `FrameController` with mock values.
    *   Render the visual elements based on the controller's state. 
